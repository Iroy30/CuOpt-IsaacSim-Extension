...
def get_prim_translation(prim):
    prim_tf = UsdGeom.Xformable(prim).ComputeLocalToWorldTransform(
...
def edge_in_volume(edge_prim, vol_prim):
    ...
    total_bounds = Gf.BBox3d(
        bbox_cache.ComputeWorldBound(vol_prim).ComputeAlignedRange()
    )

    prim_tf = UsdGeom.Xformable(edge_prim).ComputeLocalToWorldTransform(
        Usd.TimeCode()
    )
