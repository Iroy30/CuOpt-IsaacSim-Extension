...
def generate_semantic_zones(
    stage, semantic_prim_path, semantics, length, width
):
    ...
    for sematic_id, semantic_details in semantics_data.items():
        ...
        UsdGeom.PrimvarsAPI(semantic_prim).CreatePrimvar(
        ...
        shader = UsdShade.Shader(
            stage.GetPrimAtPath(f"{material_path}/Shader")
        )
        ...
        semantic_prim.CreateAttribute(
            "mfgstd:schema", Sdf.ValueTypeNames.String
        ).Set("SemanticRegion#1.0.0")
        semantic_prim.CreateAttribute(
            "mfgstd:properties:name", Sdf.ValueTypeNames.String
        ).Set(sematic_id)
        semantic_prim.CreateAttribute(
            "mfgstd:properties:semantic_type", Sdf.ValueTypeNames.String
        ).Set(semantic_details["type"])
        semantic_prim.CreateAttribute(
            "mfgstd:properties:semantic_weight", Sdf.ValueTypeNames.Float
        ).Set(semantic_details["weight"])
